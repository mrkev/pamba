...
export class GPUWaveformRenderer {
  ...
  readonly gpu: Extract<WebGPUStatus, { status: "ready" }>;
  ...
  static async create(canvas: HTMLCanvasElement, channelData: Float32Array) {
    try {
      ...
      return GPUWaveformRenderer.createPipeline(channelData, {
        status: "ready",
        adapter: adapter,
        device,
        encoder,
  ...
  static createPipeline(channelData: Float32Array, gpu: Extract<WebGPUStatus, { status: "ready" }>) {
    ...
    const waveformRenderer = new GPUWaveformRenderer(cellPipeline, channelData, gpu);
  ...
  private constructor(
    ...
    gpu: Extract<WebGPUStatus, { status: "ready" }>,
  ) {
    this.gpu = gpu;
    this.canvas = gpu.context.canvas;
    ...
    this.channelDataStorage = gpu.device.createBuffer({
      label: "Channel Data",
