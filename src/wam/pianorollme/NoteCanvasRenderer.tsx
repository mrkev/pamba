...
export class NoteCanvasRenderer {
  ...
  visibleNotesChanged(newState: NoteCanvasRenderState): boolean {
    ...
    return newState.visibleNotes.some((element, i) => !sameNote(element, nullthrows(this.oldState).visibleNotes[i]));
  }
  ...
  render(state: NoteCanvasRenderState) {
    ...
    const horizontalChanged =
      !this.oldState ||
      this.oldState.horizontalZoom != state.horizontalZoom ||
      this.oldState.position != state.position;
    ...
    let ctx = nullthrows(this.canvas.getContext("2d"));
  ...
  renderPlayhead(state: NoteCanvasRenderState) {
    let ctx = nullthrows(nullthrows(this.playhead).getContext("2d"));
    rect(ctx, 0, 0, 2, state.height, "red");
  }

  renderBackground(state: NoteCanvasRenderState) {
    let ctx = nullthrows(nullthrows(this.canvas).getContext("2d"));
    ...
    state.visibleNotes.forEach((note, i) => {
      ...
      if (note.name) {
        fillText(
          ctx,
          note.name.substring(0, 9),
          5,
          state.height - Design.cellHeight * i - 4,
          14,
          note.blackKey ? "#fff" : "#000",
        );
      }
    });
  }
